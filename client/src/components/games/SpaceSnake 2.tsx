...
const SpaceSnake: React.FC<SpaceSnakeProps> = ({ onGameStateChange }) => {
  ...
  useEffect(() => {
    ...
    // Load game over sound (pointing to actual audio file)
    gameOverSoundRef.current = new Audio('/assets/game2/enemies/Sounds/gameover.dataset/gameover.wav');
  ...
  const recordGameSessionToAPI = async (score: number) => {
    try {
      const points = calculatePointsFromScore(score, 2); // Game 2 = Space Snake
  ...
  }, [gameState.gameStatus, onGameStateChange]);
