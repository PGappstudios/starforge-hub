...
const MazeGame: React.FC<MazeGameProps> = ({ onGameEnd, onGameStateChange }) => {
  ...
  function generateEnemies(currentLevel: number = 1): CargoEnemy[] {
    const enemies: CargoEnemy[] = [];
    const currentTime = Date.now();
    const level = currentLevel;

    // Calculate enemy count based on level: 8 + (level-1) * 2
    const enemyCount = CARGO_CONFIG.BASE_ENEMY_COUNT + ((level - 1) * CARGO_CONFIG.ENEMIES_PER_LEVEL);
    ...
    // Add more spawn positions if we need more enemies than base positions
    const enemySpawnPositions = [...baseSpawnPositions];

    if (enemyCount > baseSpawnPositions.length) {
      const additionalPositions = [
        { x: GRID_CONFIG.CELL_SIZE * 8 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 3 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 21 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 3 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 8 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 16 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 21 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 16 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 3 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 6 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 26 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 6 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 3 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 13 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 26 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 13 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 12 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 4 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 17 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 15 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 6 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 9 + GRID_CONFIG.CELL_SIZE / 2 },
        { x: GRID_CONFIG.CELL_SIZE * 23 + GRID_CONFIG.CELL_SIZE / 2, y: GRID_CONFIG.CELL_SIZE * 9 + GRID_CONFIG.CELL_SIZE / 2 }
      ];
      enemySpawnPositions.push(...additionalPositions);
    }

    console.log(`Level ${level}: Spawning ${enemyCount} enemies`);
  ...
  function isWallPosition(gridX: number, gridY: number, level: number = 1): boolean {
    // Always have border walls for all levels
    ...
    switch (level) {
      case 1: return isWallPositionLevel1(gridX, gridY);
      case 2: return isWallPositionLevel2(gridX, gridY);
      case 3: return isWallPositionLevel3(gridX, gridY);
      case 4: return isWallPositionLevel4(gridX, gridY);
      case 5: return isWallPositionLevel5(gridX, gridY);
      case 6: return isWallPositionLevel6(gridX, gridY);
      case 7: return isWallPositionLevel7(gridX, gridY);
      default: return isWallPositionLevel1(gridX, gridY);
    }
  }

  function isWallPositionLevel1(gridX: number, gridY: number): boolean {
    // Level 1: L/T/I maze design (current design)

    // L-SHAPED WALLS (Corner navigation)
    // Top-left L-shape
    ...
    // T-SHAPED WALLS (Junction points)
    // Top T-shapes
    ...
    // I-SHAPED WALLS (Straight obstacles)
    // Horizontal I-shapes
  ...
  function isWallPositionLevel2(gridX: number, gridY: number): boolean {
    // Level 2: Cross and plus pattern design

    // Large cross in center-left area
    if (gridX === 8 && gridY >= 6 && gridY <= 13) return true;
    if (gridY === 9 && gridX >= 5 && gridX <= 11) return true;

    // Large cross in center-right area  
    if (gridX === 21 && gridY >= 6 && gridY <= 13) return true;
    if (gridY === 9 && gridX >= 18 && gridX <= 24) return true;

    // Corner barriers
    if (gridX >= 3 && gridX <= 5 && gridY >= 2 && gridY <= 4) return true;
    if (gridX >= 24 && gridX <= 26 && gridY >= 2 && gridY <= 4) return true;
    if (gridX >= 3 && gridX <= 5 && gridY >= 15 && gridY <= 17) return true;
    if (gridX >= 24 && gridX <= 26 && gridY >= 15 && gridY <= 17) return true;

    // Top and bottom barriers
    if (gridY === 4 && gridX >= 12 && gridX <= 17) return true;
    if (gridY === 15 && gridX >= 12 && gridX <= 17) return true;

    return false;
  }

  function isWallPositionLevel3(gridX: number, gridY: number): boolean {
    // Level 3: Diamond and hexagon pattern

    // Diamond pattern in top area
    if (gridX === 14 && gridY >= 2 && gridY <= 6) return true;
    if (gridX === 15 && gridY >= 2 && gridY <= 6) return true;
    if (gridY === 4 && gridX >= 12 && gridX <= 17) return true;

    // Hexagon pattern in bottom area
    if (gridY === 13 && gridX >= 11 && gridX <= 18) return true;
    if (gridY === 16 && gridX >= 11 && gridX <= 18) return true;
    if (gridX === 10 && gridY >= 14 && gridY <= 15) return true;
    if (gridX === 19 && gridY >= 14 && gridY <= 15) return true;

    // Side corridors with barriers
    if (gridX === 6 && gridY >= 7 && gridY <= 12) return true;
    if (gridX === 23 && gridY >= 7 && gridY <= 12) return true;

    // Scattered obstacles
    if (gridX >= 3 && gridX <= 4 && gridY === 8) return true;
    if (gridX >= 25 && gridX <= 26 && gridY === 11) return true;

    return false;
  }

  function isWallPositionLevel4(gridX: number, gridY: number): boolean {
    // Level 4: Spiral and curved patterns

    // Outer spiral starting from top-left
    if (gridX >= 3 && gridX <= 12 && gridY === 3) return true;
    if (gridX === 12 && gridY >= 3 && gridY <= 7) return true;
    if (gridX >= 6 && gridX <= 12 && gridY === 7) return true;
    if (gridX === 6 && gridY >= 7 && gridY <= 10) return true;
    if (gridX >= 6 && gridX <= 9 && gridY === 10) return true;

    // Outer spiral starting from bottom-right
    if (gridX >= 17 && gridX <= 26 && gridY === 16) return true;
    if (gridX === 17 && gridY >= 12 && gridY <= 16) return true;
    if (gridX >= 17 && gridX <= 23 && gridY === 12) return true;
    if (gridX === 23 && gridY >= 9 && gridY <= 12) return true;
    if (gridX >= 20 && gridX <= 23 && gridY === 9) return true;

    // Central barriers
    if (gridX === 14 && gridY >= 5 && gridY <= 8) return true;
    if (gridX === 15 && gridY >= 11 && gridY <= 14) return true;

    return false;
  }

  function isWallPositionLevel5(gridX: number, gridY: number): boolean {
    // Level 5: Maze-like corridors

    // Vertical corridors
    if (gridX === 5 && gridY >= 2 && gridY <= 17) return true;
    if (gridX === 10 && gridY >= 2 && gridY <= 8) return true;
    if (gridX === 10 && gridY >= 11 && gridY <= 17) return true;
    if (gridX === 19 && gridY >= 2 && gridY <= 8) return true;
    if (gridX === 19 && gridY >= 11 && gridY <= 17) return true;
    if (gridX === 24 && gridY >= 2 && gridY <= 17) return true;

    // Horizontal corridors  
    if (gridY === 5 && gridX >= 2 && gridX <= 27) return true;
    if (gridY === 14 && gridX >= 2 && gridX <= 27) return true;

    // Openings in corridors (remove walls for passages)
    if (gridX === 5 && (gridY === 6 || gridY === 9 || gridY === 13)) return false;
    if (gridX === 10 && gridY === 9) return false;
    if (gridX === 19 && gridY === 9) return false;
    if (gridX === 24 && (gridY === 6 || gridY === 9 || gridY === 13)) return false;
    if (gridY === 5 && (gridX === 7 || gridX === 12 || gridX === 17 || gridX === 22)) return false;
    if (gridY === 14 && (gridX === 7 || gridX === 12 || gridX === 17 || gridX === 22)) return false;

    return false;
  }

  function isWallPositionLevel6(gridX: number, gridY: number): boolean {
    // Level 6: Complex geometric patterns

    // Double diamond formation
    if (Math.abs(gridX - 9) + Math.abs(gridY - 6) === 3) return true;
    if (Math.abs(gridX - 20) + Math.abs(gridY - 13) === 3) return true;

    // Octagon-like structure in center
    if ((gridX === 13 || gridX === 16) && gridY >= 8 && gridY <= 11) return true;
    if ((gridY === 8 || gridY === 11) && gridX >= 13 && gridX <= 16) return true;

    // Corner triangular formations
    if (gridX + gridY <= 6) return true; // Top-left triangle
    if (gridX - gridY >= 22) return true; // Top-right triangle  
    if (gridX + gridY >= 36) return true; // Bottom-right triangle
    if (gridY - gridX >= 8) return true; // Bottom-left triangle

    return false;
  }

  function isWallPositionLevel7(gridX: number, gridY: number): boolean {
    // Level 7: Final boss level - chaotic but navigable

    // Random scattered large blocks
    const blocks = [
      {x: 4, y: 3, w: 3, h: 2}, {x: 22, y: 4, w: 2, h: 3},
      {x: 7, y: 8, w: 2, h: 4}, {x: 20, y: 7, w: 3, h: 2},
      {x: 3, y: 13, w: 4, h: 2}, {x: 23, y: 14, w: 3, h: 3},
      {x: 12, y: 2, w: 2, h: 3}, {x: 15, y: 16, w: 3, h: 2},
      {x: 8, y: 15, w: 2, h: 2}, {x: 19, y: 3, w: 2, h: 2}
    ];

    for (const block of blocks) {
      if (gridX >= block.x && gridX < block.x + block.w &&
          gridY >= block.y && gridY < block.y + block.h) {
        return true;
      }
    }
  ...
  function advanceToNextLevel(state: CargoGameState) {
    console.log(`Advancing from Level ${state.currentLevel} to Level ${state.currentLevel + 1}`);

    // Add time bonus for completing level quickly
    const levelTimeBonus = state.timeRemaining * CARGO_CONFIG.TIME_BONUS_MULTIPLIER;
    state.score += levelTimeBonus;

    // Advance to next level
    state.currentLevel++;

    // Reset level-specific properties
    state.timeRemaining = CARGO_CONFIG.GAME_DURATION; // Reset timer for new level
    state.levelStartTime = Date.now();
    state.deliveredItems = 0;
    ...
    // Generate new items and enemies for the new level
    state.availableItems = generateCargoItems();
    state.totalItems = state.availableItems.length;
    state.enemies = generateEnemies(state.currentLevel);

    // Reset CSS station
    state.cssStation.deliveredItems = [];
    state.cssStation.totalDeliveries = 0;

    // Clear particles
    state.particles = [];

    // Set status to show level transition
    state.gameStatus = 'levelComplete';

    console.log(`Level ${state.currentLevel} initialized with ${state.enemies.length} enemies and ${state.totalItems} items`);
  }
  ...
  // Game loop update function (moved before useGameLoop to fix initialization error)
    ...
    setGameState(prev => {
      ...
      if (newState.timeRemaining <= 0) {
        ...
        if (onGameEnd) {
          ...
          const finalScore = baseScore; // no time bonus when time runs out
      ...
      if (newState.deliveredItems >= newState.totalItems) {
        if (newState.currentLevel < newState.totalLevels) {
          advanceToNextLevel(newState);
        } else {
          newState.gameStatus = 'gameComplete';
          onGameStateChange?.(false);
        }
      }

      // (Projectiles system removed in this version)

      return newState;
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [gameState.gameStatus, onGameEnd, onGameStateChange, recordGameSessionToAPI]);
  ...
  function drawMazePathways(ctx: CanvasRenderingContext2D) {
    ...
    for (let gridX = 0; gridX < GRID_CONFIG.GRID_WIDTH; gridX++) {
      for (let gridY = 0; gridY < GRID_CONFIG.GRID_HEIGHT; gridY++) {
        ...
        if (distanceFromCenter > 3) {
          if (isWallPosition(gridX, gridY)) {
            // Draw wall block with level-specific color
            ctx.fillStyle = levelColors.base;
            ctx.fillRect(x, y, GRID_CONFIG.WALL_SIZE, GRID_CONFIG.WALL_SIZE);

            // Add level-specific edges with flickering
            const flicker = Math.sin(time * 3 + gridX * 0.5 + gridY * 0.5) * 0.3 + 0.7;

            // Helper function to replace all FLICKER placeholders
            const replaceFlickerValues = (colorString: string, flickerValue: number) => {
              return colorString
                .replace(/FLICKER_(\d+)/g, (match, num) => String(Math.floor(parseInt(num) * flickerValue)))
                .replace('FLICKER', String(flickerValue));
            };

            ctx.strokeStyle = replaceFlickerValues(levelColors.edge, flicker);
            ctx.lineWidth = 2;
            ctx.strokeRect(x, y, GRID_CONFIG.WALL_SIZE, GRID_CONFIG.WALL_SIZE);

            // Add inner glow
            ctx.strokeStyle = replaceFlickerValues(levelColors.glow, flicker * 0.6);
            ctx.lineWidth = 1;
            ctx.strokeRect(x + 2, y + 2, GRID_CONFIG.WALL_SIZE - 4, GRID_CONFIG.WALL_SIZE - 4);

            // Add corner highlights when flickering is strong
            if (flicker > 0.8) {
              ctx.fillStyle = replaceFlickerValues(levelColors.highlight, (flicker - 0.8) * 2);
  ...
  function getLevelColors(level: number) {
    switch (level) {
      case 1: // Red (current)
        return {
          base: '#440000',
          edge: 'rgba(255, FLICKER_80, 0, FLICKER)',
          glow: 'rgba(255, FLICKER_180, 20, FLICKER)',
          highlight: 'rgba(255, 200, 0, FLICKER)'
        };
      case 2: // Blue  
        return {
          base: '#000044',
          edge: 'rgba(0, FLICKER_120, 255, FLICKER)',
          glow: 'rgba(20, FLICKER_180, 255, FLICKER)',
          highlight: 'rgba(100, 200, 255, FLICKER)'
        };
      case 3: // Yellow
        return {
          base: '#444400',
          edge: 'rgba(255, FLICKER_200, 0, FLICKER)',
          glow: 'rgba(255, FLICKER_220, 20, FLICKER)',
          highlight: 'rgba(255, 255, 100, FLICKER)'
        };
      case 4: // Purple
        return {
          base: '#440044',
          edge: 'rgba(FLICKER_150, 0, 255, FLICKER)',
          glow: 'rgba(FLICKER_180, 20, 255, FLICKER)',
          highlight: 'rgba(200, 100, 255, FLICKER)'
        };
      case 5: // Green
        return {
          base: '#004400',
          edge: 'rgba(0, 255, FLICKER_80, FLICKER)',
          glow: 'rgba(20, 255, FLICKER_120, FLICKER)',
          highlight: 'rgba(100, 255, 200, FLICKER)'
        };
      case 6: // White
        return {
          base: '#444444',
          edge: 'rgba(FLICKER_200, FLICKER_200, FLICKER_200, FLICKER)',
          glow: 'rgba(FLICKER_255, FLICKER_255, FLICKER_255, FLICKER)',
          highlight: 'rgba(255, 255, 255, FLICKER)'
        };
      case 7: // Orange
        return {
          base: '#442200',
          edge: 'rgba(255, FLICKER_140, 0, FLICKER)',
          glow: 'rgba(255, FLICKER_180, 40, FLICKER)',
          highlight: 'rgba(255, 180, 0, FLICKER)'
        };
      default:
        return getLevelColors(1);
    }
  }

  function getLevelDescription(level: number): string {
    switch (level) {
      case 1: return "Red fire blocks with L/T/I maze patterns";
      case 2: return "Blue ice blocks with cross and plus patterns";  
      case 3: return "Yellow energy blocks with diamond and hexagon shapes";
      case 4: return "Purple void blocks with spiral and curved patterns";
      case 5: return "Green bio blocks with complex corridor mazes";
      case 6: return "White crystal blocks with geometric formations";
      case 7: return "Orange plasma blocks with chaotic boss-level layout";
      default: return "Unknown level configuration";
    }
  }
  ...
  function drawUI(ctx: CanvasRenderingContext2D) {
    ...
    if (gameState.gameStatus === 'playing') {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(`Level ${gameState.currentLevel}/${gameState.totalLevels}`, 85, 25);

      ctx.fillStyle = '#ffffff';
      ctx.font = 'bold 12px Arial';
      ctx.fillText(`Score: ${gameState.score}`, 85, 40);

      ctx.fillStyle = '#cccccc';
      ctx.font = '10px Arial';
      ctx.fillText(`Items: ${gameState.deliveredItems}/${gameState.totalItems}`, 85, 55);
      ...
      <CardContent className="p-4">
        <div className="flex flex-col items-center space-y-4">
          <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/80 rounded-lg">
                <div className="text-center text-white">
                  ...
                  <div className="mb-6 text-sm text-gray-400 space-y-1">
                    <p>🏆 <span className="text-purple-400 font-bold">7 PROGRESSIVE LEVELS</span> - Master them all!</p>
                    <p>⏱️ <span className="text-orange-400 font-bold">5 minutes per level</span> - Race against time!</p>
                    <p>💯 <span className="text-green-400 font-bold">100 points</span> per item delivered to CSS</p>
                    <p>⚡ <span className="text-blue-400 font-bold">10 points</span> per remaining second as time bonus</p>
                    <p>👹 <span className="text-red-400 font-bold">8-20 enemies</span> - 2 more per level!</p>
                    <p>🎨 <span className="text-cyan-400 font-bold">Colored blocks</span> - Red→Blue→Yellow→Purple→Green→White→Orange</p>
                    <p>🧩 <span className="text-yellow-400 font-bold">Unique layouts</span> per level</p>
                  ...
                  >
                    {imagesLoading ? 'LOADING...' : !images ? 'ASSETS ERROR' : 'START MISSION - 1 Credit'}
                  </Button>
                </div>
              </div>
            )}
            ...
            {gameState.gameStatus === 'levelComplete' && gameState.currentLevel <= gameState.totalLevels && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/90 rounded-lg">
                <div className="text-center text-white">
                  {gameState.currentLevel <= gameState.totalLevels ? (
                    <>
                      <h2 className="text-4xl font-bold mb-4 text-green-400 drop-shadow-[0_0_20px_rgba(34,197,94,0.8)]">
                        LEVEL {gameState.currentLevel - 1} COMPLETE!
                      </h2>
                      <div className="mb-6 space-y-2">
                        <p className="text-xl font-bold text-yellow-400">
                          Current Score: {gameState.score.toLocaleString()}
                        </p>
                        ...
                        <div className="mt-4 p-3 bg-purple-600/20 border border-purple-600/50 rounded-lg">
                          <p className="text-purple-300 text-sm">
                            New level features: {getLevelDescription(gameState.currentLevel)}
                          </p>
                          <p className="text-orange-400 text-sm font-bold">
                            {CARGO_CONFIG.BASE_ENEMY_COUNT + ((gameState.currentLevel - 1) * CARGO_CONFIG.ENEMIES_PER_LEVEL)} enemies incoming!
                          </p>
                        </div>
                      </div>
                      <div className="space-x-4">
            ...
            {gameState.gameStatus === 'gameComplete' && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/95 rounded-lg">
                <div className="text-center text-white">
                  <h2 className="text-6xl font-bold mb-6 text-yellow-400 drop-shadow-[0_0_30px_rgba(250,204,21,0.8)] animate-pulse">
                    🏆 VICTORY! 🏆
                  </h2>
                  ...
                  <div className="mb-6 space-y-3">
                    ...
                    <p className="text-xl text-blue-400">
                      Final Time Bonus: {gameState.timeRemaining * CARGO_CONFIG.TIME_BONUS_MULTIPLIER} points
                    </p>
                    <div className="mt-4 p-4 bg-gradient-to-r from-purple-600/30 to-blue-600/30 border border-yellow-400/50 rounded-lg">
                      <p className="text-yellow-300 text-lg font-bold">🌟 STAR SEEKERS CHAMPION 🌟</p>
                      <p className="text-purple-300 text-sm">
                        You've mastered all 7 cosmic cargo delivery challenges!
                      </p>
                      <p className="text-blue-300 text-sm">
                        From 8 enemies to 20 enemies across 7 unique maze layouts
                      </p>
                    </div>
                  </div>
                  <div className="space-x-4">
                    <Button onClick={resetGame} className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-lg px-8 py-3 font-bold">
                      PLAY AGAIN - 1 Credit
                    </Button>
                    <Button onClick={() => navigate('/dashboard')} variant="outline" className="text-lg px-8 py-3 border-yellow-400 text-yellow-400 hover:bg-yellow-400/10">
                      MAIN MENU
                    </Button>
                  </div>
                </div>
