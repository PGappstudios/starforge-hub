...
const MazeGame: React.FC<MazeGameProps> = ({ onGameEnd, onGameStateChange }) => {
  ...
  function generateEnemies(): CargoEnemy[] {
    ...
    for (let i = 0; i < CARGO_CONFIG.ENEMY_COUNT; i++) {
      ...
      // Validate spawn position is safe (not on walls)
      if (!isPositionSafeForItem(spawnPosition.x, spawnPosition.y, 1)) {
        // Find alternative safe spawn position
        let found = false;
        for (let radius = 1; radius <= 5 && !found; radius++) {
          for (let offsetX = -radius; offsetX <= radius && !found; offsetX++) {
            for (let offsetY = -radius; offsetY <= radius && !found; offsetY++) {
              ...
              if (isPositionSafeForItem(testX, testY, 1)) {
  ...
  function updateEnemies(state: CargoGameState, deltaTime: number) {
    ...
    state.enemies.forEach(enemy => {
      ...
      if (enemy.isChasing) {
        ...
        // Move more frequently when chasing for smoother following
        const moveInterval = 16; // ~60fps movement
        ...
        if (distanceToTarget < 30) {
          ...
          enemy.targetPosition = enemy.patrolPoints[enemy.currentPatrolIndex];
        }

        // Move along patrol route
        const patrolInterval = 50; // Consistent patrol movement
        if (currentTime - enemy.lastMoveTime > patrolInterval) {
          moveEnemyTowardsTarget(enemy, state);
          enemy.lastMoveTime = currentTime;
        }
      }
  ...
  function respawnEnemies(state: CargoGameState) {
    ...
    state.enemies.forEach((enemy, index) => {
      if (!enemy.active && respawnedCount < state.killedEnemies) {
        ...
        if (playerDistance < 100 || !isPositionSafeForItem(spawnPosition.x, spawnPosition.y, 1)) {
          // Find alternative safe spawn position
          let found = false;
          for (let radius = 1; radius <= 5 && !found; radius++) {
            for (let offsetX = -radius; offsetX <= radius && !found; offsetX++) {
              for (let offsetY = -radius; offsetY <= radius && !found; offsetY++) {
                ...
                if (testPlayerDistance >= 100 && isPositionSafeForItem(testX, testY, 1)) {
                  spawnPosition = { x: testX, y: testY };
                  found = true;
                }
              }
  ...
  function checkCSSDelivery(state: CargoGameState) {
    ...
    if (distance < cssStation.radius && player.cargo.items.length > 0) {
      ...
      const totalPoints = deliveredItems.reduce((sum, item) => sum + item.points, 0);
      ...
      console.log(`Delivered ${deliveredItems.length} items for ${totalPoints} points. Total delivered: ${state.deliveredItems}/${state.totalItems}`);
    }
  }
  ...
  function drawUI(ctx: CanvasRenderingContext2D) {
    // No UI elements drawn on canvas - all information moved to results area
  }
