    ...
    <div className={`min-h-screen ${isPlaying ? 'bg-black' : 'cosmic-bg'}`}>
      ...
      <div className={`flex justify-center ${isPlaying ? 'min-h-screen items-center' : ''}`}>
        {canAfford(GAME_COST) ? (
            ...
            <SpaceShooter 
              onGameStateChange={(isPlaying) => {
                ...
                if (isPlaying && !hasPlayedGame) {
                  ...
                  spendCredits(GAME_COST, 'Cosmic Battle Arena - Game Started');
                }
              }}
              onGameEnd={(score) => {
                if (!gameEnded) {
                  setGameEnded(true);
                  // SpaceShooter handles score submission internally via recordGameSessionToAPI
                }
              }} 
            />
          </>
          ...
          !isPlaying && (
            <Card className="max-w-md mx-auto bg-gray-800/20 border-gray-600/50">
              <CardContent className="pt-6 text-center">
                <p className="text-gray-300">
                  You need {GAME_COST} credit to play this space shooter game.
                </p>
              </CardContent>
            </Card>
          )
